...
int main(int argc, char* argv[])
{
	...
	unsigned char frozen_flag[32], flag[32];
	fread(&frozen_flag, sizeof(unsigned char), 32, ptr_myfile);

	ICE_KEY *ice = ice_key_create(1);
	ice_key_set(ice, "ice-cold");
	
	for (int i = 0; i < 32; i += 8)
	{
		ice_key_decrypt(ice, &(frozen_flag[i]), &(flag[i]));
	}
